// Package codex provides an OpenAI Codex implementation of the ModelProvider interface.
package codex
...
// Config holds configuration for the Codex provider.
type Config struct {
	// APIKey is the OpenAI API key.
	APIKey string
	// BaseURL overrides the default OpenAI API base URL.
	BaseURL string
	// Model specifies the model to use (default: gpt-4o).
	Model string
...
// CodexProvider implements mg.ModelProvider for OpenAI Codex/GPT models.
...
// NewCodexProvider constructs a new CodexProvider with the given configuration.
func NewCodexProvider(cfg Config, logger *zap.Logger) *CodexProvider {
	baseURL := cfg.BaseURL
	if baseURL == "" {
...
func (p *CodexProvider) ProviderName() string {
	return "openai-codex"
}

// HealthCheck verifies the provider is reachable by listing available models.
func (p *CodexProvider) HealthCheck(ctx context.Context) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, p.baseURL+"/models", nil)
	if err != nil {
		return fmt.Errorf("codex: health check request: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("codex: health check: unexpected status %d", resp.StatusCode)
	}
	return nil
}
...
func (p *CodexProvider) Classify(ctx context.Context, riskCtx mg.RiskContext) (*mg.ClassificationResult, error) {
	prompt := fmt.Sprintf(
		"Classify the risk level for the following scores and return JSON: risk_level (low|medium|high|critical), risk_score (0-100), confidence (0-1), rationale (string).\n\nAnomaly: %.2f, PolicyViolation: %.2f, ThreatIntel: %.2f, AssetCriticality: %.2f\nContext: %s",
		riskCtx.AnomalyScore, riskCtx.PolicyViolationScore, riskCtx.ThreatIntelScore, riskCtx.AssetCriticalityScore, riskCtx.Context,
	)
	raw, err := p.chatCompletion(ctx, prompt)
	if err != nil {
		return nil, fmt.Errorf("codex: classify: %w", err)
	}
	var result struct {
		RiskLevel  string  `json:"risk_level"`
...
func (p *CodexProvider) ProposeActions(ctx context.Context, incidentCtx mg.IncidentContext) (*mg.ActionProposal, error) {
	prompt := fmt.Sprintf(
		"Propose response actions for the following security incident. Return JSON: actions (array of {id,type,target,rationale,risk_if_not_taken}), blast_radius (string), rollback_plan (string), requires_human_approval (bool).\n\nIncident: %s\nTier: %s\nRisk Score: %.2f\nAffected: %v",
		incidentCtx.EventSummary, incidentCtx.Tier, incidentCtx.RiskScore, incidentCtx.AffectedResources,
	)
	raw, err := p.chatCompletion(ctx, prompt)
	if err != nil {
		return nil, fmt.Errorf("codex: propose actions: %w", err)
	}
	var result struct {
		Actions []struct {
			ID             string `json:"id"`
...
func (p *CodexProvider) Explain(ctx context.Context, decisionCtx mg.DecisionContext) (*mg.Explanation, error) {
	prompt := fmt.Sprintf(
	...
	raw, err := p.chatCompletion(ctx, prompt)
	if err != nil {
		return nil, fmt.Errorf("codex: explain: %w", err)
	}
	var result struct {
...
// chatCompletion calls the OpenAI Chat Completions API with retry logic.
func (p *CodexProvider) chatCompletion(ctx context.Context, prompt string) (string, error) {
	...
	for attempt := 0; attempt < p.maxRetries; attempt++ {
		...
		resp, err := p.httpClient.Do(req)
		if err != nil {
			lastErr = fmt.Errorf("codex: http request (attempt %d): %w", attempt+1, err)
			p.logger.Warn("codex: request failed, retrying", zap.Int("attempt", attempt+1), zap.Error(err))
			continue
		}
		defer resp.Body.Close()

		if resp.StatusCode == http.StatusTooManyRequests {
			lastErr = fmt.Errorf("codex: rate limited (attempt %d)", attempt+1)
			p.logger.Warn("codex: rate limited", zap.Int("attempt", attempt+1))
			continue
		}
		if resp.StatusCode != http.StatusOK {
			b, _ := io.ReadAll(resp.Body)
			return "", fmt.Errorf("codex: unexpected status %d: %s", resp.StatusCode, string(b))
		}
		...
		if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
			return "", fmt.Errorf("codex: decode response: %w", err)
		}
		if len(response.Choices) == 0 {
			return "", fmt.Errorf("codex: no choices in response")
		}
		return response.Choices[0].Message.Content, nil
	}
	return "", fmt.Errorf("codex: all retries exhausted: %w", lastErr)
}
